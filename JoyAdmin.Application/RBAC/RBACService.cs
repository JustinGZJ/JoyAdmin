...
using JoyAdmin.Application.RBAC.Dtos;
using JoyAdmin.Core;
using Mapster;
...
namespace JoyAdmin.Application.RBAC;
...
public class RBACService : IDynamicApiController
{
    ...
    {
        // 验证用户名和密码
        var user = _userRepository.FirstOrDefault(u => u.Account.Equals(input.Account)) ??
                   throw Oops.Oh(ErrorCode.WrongUser).StatusCode(ErrorStatus.ValidationFaild);
    ...
    public UserDto GetUserInfo()
    {
        ...
        entity.Roles.SelectMany(r => r.Securities).ToList().ForEach(s => { user.Access.Add(s.UniqueCode); });
        return user;
    }
    ...
    public Task SaveAuth(SecurityDto input)
    {
        if (input.Id == 0)
        {
            var hasData = _securityRepository
                .Where(m => m.UniqueCode == input.UniqueCode || m.UniqueName == input.UniqueName).FirstOrDefault();
            if (hasData != null)
            {
                ...
                if (hasData.UniqueName == input.UniqueName)
        ...
        {
            var hasData = _securityRepository.Where(m =>
                    m.Id != input.Id && (m.UniqueCode == input.UniqueCode || m.UniqueName == input.UniqueName))
                .FirstOrDefault();
            if (hasData != null)
            {
                if (hasData.UniqueCode == input.UniqueCode)
    ...
    public Task DeleteEmployee(long id)
    {
        // 获取用户Id
        var userId = _authorizationManager.GetUserId();
        if (userId == id) throw Oops.Oh(ErrorCode.WrongValidation, "不能删除当前账号").StatusCode(ErrorStatus.ValidationFaild);
    ...
    public Task GiveUserRole(UserRoleInput input)
    {
        ...
        foreach (var roleid in input.RoleIds) list.Add(new UserRole { UserId = input.UserId, RoleId = roleid });
    ...
    public Task UpdatePwd(UpdatePwdInput input)
    {
        ...
        return _userRepository.UpdateIncludeAsync(user, new[] { "Password" });
    }
