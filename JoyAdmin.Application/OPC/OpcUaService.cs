...
public class OpcUaService:IDynamicApiController
{
    ...
    public OpcUaService(IOpcUaClientWrapper opcUaClient,IOpcMonitorDataStorage dataStorage,IConfiguration configuration)
    {
    ...
    {
         ReferenceDescription[] referenceDescriptions=  _opcUaClient.GetAllRelationNodeOfNodeId( nodeId);
    ...
    public IEnumerable<string> MonitAllRelationNodeOfNodeId(string nodeId)
    {
       ...
       var nodeids =referenceDescriptions.Select(x => x.NodeId.ToString()).ToArray();
           _opcUaClient.BatchNodeIdDatasSubscription(nodeId,nodeids,(
          (s, item, arg3) =>
          {
